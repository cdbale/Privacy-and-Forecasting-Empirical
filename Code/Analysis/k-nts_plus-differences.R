...
fp <- "../../Data/Cleaned/M3/"
features_path <- "../../Data/Features/M3/"
# path to files with error distributions
ed_file_path <- "../../Outputs/Results/M3/Error_Distributions/"
...
X <- import_data(file_name="monthly-OTHER_h1_train.csv", file_path=fp, sp=12)
...
feature_file_names <- grep("h2_train", list.files("../../Data/Features/M3/"), value=TRUE)
feature_file_names <- grep("monthly-OTHER", feature_file_names, value=TRUE)
original_feature_file_name <- grep("AN_", feature_file_names, value=TRUE, invert=TRUE)
original_feature_file_name <- grep("DP_", original_feature_file_name, value=TRUE, invert=TRUE)
feature_file_names <- c(grep("AN_", feature_file_names, value=TRUE), grep("DP_", feature_file_names, value=TRUE))
...
for (f in c(feature_file_names, original_feature_file_name)){
...
# all forecasts
all_fcasts <- list.files("../../Outputs/Forecasts/M3/")

# filter for just the ones using h2
all_fcasts <- grep("_h2_", all_fcasts, value=TRUE)
# filter for just the ones for the current file
all_fcasts <- grep("monthly-OTHER", all_fcasts, value=TRUE)

original_fcasts <- grep("_AN_", all_fcasts, value=TRUE, invert=TRUE)
original_fcasts <- grep("_DP_", original_fcasts, value=TRUE, invert=TRUE)

protected_fcasts <- c(grep("_AN_", all_fcasts, value=TRUE), grep("_DP_", all_fcasts, value=TRUE))

all_fcasts <- tibble()

for (f in c(protected_fcasts, original_fcasts)){
  
  temp_fcasts <- unname(as_vector(read_csv(paste0("../../Outputs/Forecasts/M3/", f))))
  
  finfo <- strsplit(f, split="_")[[1]]
  
  
  
  if (!finfo[3] %in% c("AN", "DP")){
    fcasts <- tibble(Model=finfo[1],
                     Data=finfo[3],
                     Protection="Original",
                     Parameter="Original",
  ...
  all_fcasts <- bind_rows(all_fcasts, fcasts)
  
}

## join features to forecasts
split_fcasts <- all_fcasts %>% 
...
evals <- lapply(full_data_scaled, function(x) attrEval("Forecasts", data=x, estimator="RReliefFexpRank"))
...
for (i in seq_along(full_diff_data)){
...
for (i in seq_along(full_diff_data)){
  ...
  df <- full_diff_data[[i]][,c("fcast_diff", top_feats[[i]])]
  
  rf_res <- ranger(fcast_diff ~ ., data=df, importance="permutation", num.trees=500)
...
for (f in file_names[6]){
  ...
  for (j in c(3)){
    ...
    write.csv(X_knts, file=paste0(fp, "k-nts-plus-diffs_", j, "_", f))
