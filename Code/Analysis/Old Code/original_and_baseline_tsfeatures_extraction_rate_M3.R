...
file_path <- "../../Data/Cleaned/M3/"

# import names of original and baseline protected data files
file_names <- grep("rate_", list.files(file_path), value=TRUE)
file_names <- grep("_train.csv", file_names, value=TRUE)

# includes all files names for rate data in its original form or protected
# using additive noise or differential privacy. Includes horizons h1 and h2
...
for (f in file_names){
  ...
  write.csv(features, file=paste0("../../Data/Features/M3/", "features_", f), row.names=FALSE)
}
