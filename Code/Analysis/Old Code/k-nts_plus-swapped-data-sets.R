...
# we are going to experiment with using swapped data sets as the baseline
# privacy method data sets.

# Let's test with one small data set so it is fast.

qf_data <- read.csv("../../Data/Cleaned/quarterly-FINANCE_h2_train.csv")


# loop through time periods, randomly swapping across series. we had 20 protected
# data sets before (5 params for AN and 5 for DP) so let's make 20 swapped.

swapped_baseline_df <- list()

for (i in 1:20){
  
  baseline_swapped <- apply(qf_data, 2, function(x) sample(x, size=nrow(qf_data), replace=FALSE))
  
  write.csv(baseline_swapped, paste0("../../Data/Cleaned/baseline-swapped_", i, "_quarterly-FINANCE_h2_train.csv"))
...
X <- import_data(file_name="quarterly-FINANCE_h1_train.csv", file_path=fp, sp=4)

# file with errors for all models on original data
eds <- read_csv(paste0(ed_file_path, "quarterly-FINANCE_all_distributions_h2.csv"))
...
eds <- eds %>% gather(key="name", value="values") %>%
  ...
  mutate(Protection = if_else(is.na(Protection), "Original", ifelse(Protection == "quarterly-FINANCE", "Original", Protection)),
         Parameter = if_else(is.na(Parameter), "Original", Parameter)) %>%
  select(-Data) %>%
  filter(Protection %in% c("Original", "baseline-swapped"),
         !Model %in% c("ARIMA", "RNN"))
...
original_features <- read.csv("../../Data/Features/features_quarterly-FINANCE_h2_train.csv")

original_features["Protection"] <- "Original"
original_features["Parameter"] <- "Original"

full_features <- as_tibble(original_features)

# import protected features and assign new variable values for linking to errors
for (i in 1:20){
  
  protected_data <- read_csv(paste0("../../Data/Cleaned/baseline-swapped_", i, "_quarterly-FINANCE_h2_train.csv"))
  
  features <- extract_features(protected_data, sp=sp_l, feature_vector=fv)
  
  features <- features %>% select(-nperiods, -seasonal_period)
  
  features["Protection"] <- "baseline-swapped"
  features["Parameter"] <- as.character(i)
