...
knts_alg <- function(time_series, sp, window_length, kvals, features_to_calculate, selected_features, importance_weights, corr_based=FALSE){
...
perform_knts <- function(ts_file, ts_file_path, seasonal_period, window_length, kvals, features_to_calculate, selected_features, importance_weights, corr_based=FALSE){
  ...
  X_protected <- lapply(1:length(Xs), function(x) knts_alg(Xs[[x]], sp=seasonal_period, window_length=window_length, kvals=kvals, features_to_calculate=features_to_calculate, selected_features=selected_features[[x]], importance_weights=importance_weights[[x]], corr_based=corr_based))
...
for (f in file_names){
  ...
  computation_row <- tibble(File = f,
                            ...
                            Swap = difftime(swap_stop, swap_start, units="secs"))
