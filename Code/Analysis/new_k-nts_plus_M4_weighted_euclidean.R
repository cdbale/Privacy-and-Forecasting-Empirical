...
for (f in file_names[1]){
  ...
  fsr <- feature_selection(scaled_feature_data=full_data_scaled, num_rfe_iters=num_iter, models=models)
  ...
  for (j in c(3)){
    ...
    write.csv(X_knts, file=paste0(fp, "k-nts-plus-M4-weighted-euclidean_", j, "_", f), row.names=FALSE)
