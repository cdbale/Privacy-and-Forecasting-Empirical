...
# To reduce computational complexity, we are only using'
...
knts_alg <- function(time_series, sp, window_length, kvals, features_to_calculate, selected_features, importance_weights){
  ...
  if (length(importance_weights[to_keep]) > 1){
...
perform_knts <- function(ts_file, ts_file_path, seasonal_period, window_length, kvals, features_to_calculate, selected_features, importance_weights){
  ...
  X_k <- lapply(X_k, function(x) lapply(x, exp))
...
for (f in file_names[6]){
  ...
  if (window_length <= sp){
    for (i in seq_along(sf)){
      imp_weights[[i]] <- imp_weights[[i]][!sf[[i]] %in% c("seasonal_strength", "peak", "trough", "seas_acf1", "seas_pacf")]
      sf[[i]] <- sf[[i]][!sf[[i]] %in% c("seasonal_strength", "peak", "trough", "seas_acf1", "seas_pacf")]
    }
  }
