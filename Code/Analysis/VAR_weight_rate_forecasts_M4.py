...
# VAR simulated files
var_file_path = "../../Outputs/VAR Simulated/M4_rate/"
var_sim_files = os.listdir(var_file_path)
var_sim_files = [x for x in var_sim_files if "h1_train" in x and not any(y in x for y in ['k-nts', 'AN_', 'DP_', '.DS_Store'])]
...
for train_file in h1_train_files:
    ...
    if "Monthly" in train_file:
        ...
        sp = 4
    elif "Weekly" in train_file:
        sp = 52
    elif "Hourly" in train_file:
        sp = 24
    elif "Daily" in train_file:
        sp = 7
    else:
        sp = 1
    ...
    knts_file = [x for x in knts_train_files if x[13:-9] in train_file]
    [knts_file] = knts_file
    ...
    sim_train = pd.read_csv("../../Outputs/VAR Simulated/M4_rate/" + sim_file, header=None, skiprows=1)
