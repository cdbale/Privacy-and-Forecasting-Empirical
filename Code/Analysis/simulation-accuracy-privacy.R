...
orig_features_g <- tsfeatures(tslist=g_simulated_series_train, features=fv, scale=FALSE)
orig_features_b <- tsfeatures(tslist=b_simulated_series_train, features=fv, scale=FALSE)

an_features_g <- tsfeatures(tslist=lapply(g_simulated_series_train, function(x) additive_noise(x, s=svals[2])), features=fv, scale=FALSE)
an_features_b <- tsfeatures(tslist=lapply(b_simulated_series_train, function(x) additive_noise(x, s=svals[2])), features=fv, scale=FALSE)

dp_features_g <- tsfeatures(tslist=lapply(g_simulated_series_train, function(x) differential_privacy(x, epsilon=epsvals[1])), features=fv, scale=FALSE)
dp_features_b <- tsfeatures(tslist=lapply(b_simulated_series_train, function(x) differential_privacy(x, epsilon=epsvals[1])), features=fv, scale=FALSE)

temp_knts_g <- as.list(as.data.frame(knts_alg(time_series=g_simulated_series_train, sp=1, window_length=25, k=kvals[8], features_to_calculate=fv, selected_features=fs_g$selected_features)))
temp_knts_b <- as.list(as.data.frame(knts_alg(time_series=b_simulated_series_train, sp=1, window_length=25, k=kvals[8], features_to_calculate=fv, selected_features=fs_b$selected_features)))

temp_knts_g <- lapply(temp_knts_g, function(x) ts(x, frequency=sp))
temp_knts_b <- lapply(temp_knts_b, function(x) ts(x, frequency=sp))

knts_features_g <- tsfeatures(tslist=temp_knts_g, features=fv, scale=FALSE)
knts_features_b <- tsfeatures(tslist=temp_knts_b, features=fv, scale=FALSE)
...
features_b <- tsfeatures(tslist = b_simulated_series_train, 
                         features = c("series_mean", "kurtosis", "series_variance", "stl_features"),
                         scale=FALSE)[,fs_b$selected_features]

features_g <- tsfeatures(tslist = g_simulated_series_train, 
                         features = c("series_mean", "unitroot_kpss", "series_variance", "stl_features", "max_var_shift"),
                         scale=FALSE)[,fs_g$selected_features]

pca_features_b <- prcomp(features_b)
pca_features_g <- prcomp(features_g)

as_tibble(pca_features_b$x[,1:2]) %>%
  bind_rows(as_tibble(pca_features_g$x[,1:2])) %>%
  mutate(Set = rep(c("Low Autocorrelation/High Spectral Entropy", "High Autocorrelation/Low Spectral Entropy"), each=nrow(pca_features_b$x))) %>%
