...
for h in horizons:
    ...
    # taking the first future point from test data for evaluating h=1,
    # taking all 18 points for evaluationg h=18
    test = pd.read_csv(forecasts_path + "Test_" + "h18" + ".csv")
    test = test.iloc[:int(h[1:]),:]

    # create list of file names of original forecasts for each model
    original_fcast_names = [f for f in fcast_files if "m3_original_fcasts_"+h+"_" in f]
    ...
    data_dict[h] = smape_original

dd = pd.DataFrame(data_dict)

dd.to_csv(results_path + "m3_original" + "_sMAPE.csv", index=False)
    #
