...
var_sim_files <- list.files("../../Outputs/VAR Simulated/M3_rate/")
# make sure protected versions are excluded
...
var_sim_files <- grep("rate", var_sim_files, value=TRUE)
  ...
  for (j in seq_along(params)){
    ...
    for (f in seq_along(pm_files)){
      ...
      if (current_name == "VAR-simulated"){
        ...
        confidential_file_prefix <- paste0(strsplit(pm_files[f], "_")[[1]][1:2], collapse="_")
        
        # import the protected series
        X <- process_series(read.csv(paste0(fp, grep(confidential_file_prefix, file_names, value=TRUE))))
        
        # import the protected series
        Xp <- process_series(read.csv(paste0("../../Outputs/VAR Simulated/M3_rate/", pm_files[f])))
        ...
        confidential_file_prefix <- paste0(strsplit(pm_files[f], "_")[[1]][3:4], collapse="_")
