...
def import_split_save(time_series_frequency, min_number_series):
    ...
    temp_train = [np.log(x) for x in temp_train]
    temp_test = [np.log(x) for x in temp_test]
        
    # convert to rates
    temp_test = [(x.iloc[0] - temp_train[i].iloc[-1])/np.mean([x.iloc[0], temp_train[i].iloc[-1]]) for i,x in enumerate(temp_test)]
    temp_train = [rate_conversion(x) for x in temp_train]
    ...
    test_1 = pd.DataFrame(temp_test)

    # now save all data sets
    train_2.to_csv("../../Data/Cleaned/M4_rate/" + time_series_frequency + "_h2_train" + ".csv", index=False)
    train_1.to_csv("../../Data/Cleaned/M4_rate/" + time_series_frequency + "_h1_train" + ".csv", index=False)
    test_2.to_csv("../../Data/Cleaned/M4_rate/" + time_series_frequency + "_h2_test" + ".csv", index=False)
    test_1.to_csv("../../Data/Cleaned/M4_rate/" + time_series_frequency + "_h1_test" + ".csv", index=False)
    
[import_split_save(freq, 16) for freq in ["Yearly", "Quarterly", "Monthly", "Weekly", "Daily", "Hourly"]]