...
all_tsne <- all_tsne %>%
  mutate(dataid = as.character(dataid),
         data_subset = paste0(data, "_", dataid))
```

```{r}
all_tsne %>% group_by(data_subset) %>% summarize(AvgNeighborDistance = mean(AvgNeighborDistance))
```
...
```{r}
all_acc_results <- all_original_results %>% left_join(all_protected_results, by=c("Data", "Snum", "Horizon", "Model", "Denominators"))
...
```{r}
acc_tsne <- bind_cols(all_acc_results, all_tsne[,c("SpecEntropy", "AvgNeighborDistance")]) %>%
  mutate(MASEDiff = protected_MASE - original_MASE)
...
```{r}
summary(lm(MASEDiff ~ SpecEntropy + AvgNeighborDistance + I(SpecEntropy*AvgNeighborDistance) + Data, data=acc_tsne))
```


```{r}
acc_tsne %>%
  ggplot(aes(x=MASE_diff, y=AvgNeighborDistance)) +
  geom_point()
```
...
all_original_results <- all_original_results %>%
  group_by(Data, Snum) %>%
  summarize(AvgMAE = mean(values),
            AvgMASE = mean(MASE), .groups='drop')

all_protected_results <- all_protected_results %>%
  filter(Protection == "k-nts-plus", Parameter == 3) %>%
  group_by(Data, Snum) %>%
  summarize(AvgProtectedMAE = mean(values),
            AvgProtectedMASE = mean(MASE), .groups='drop')
```
...
```{r}
tsne_acc <- bind_cols(all_original_results, all_protected_results[,c("AvgProtectedMAE", "AvgProtectedMASE")], all_tsne)
```
...
plt <- tsne_acc %>%
  ...
  facet_wrap(~data_subset, scales='free')
...
```{r}
tsne_acc %>%
  filter(data_subset=="other-OTHER_1") %>%
  ggplot(aes(x=Dim1, y=Dim2, color=SpecEntropy)) +
  geom_point(size=1.75)
```
...
```{r}
tsne_acc_priv <- tsne_acc %>%
  group_by(data_subset) %>%
  summarize(AvgMAE = mean(AvgMAE),
            AvgMASE = mean(AvgMASE),
            AvgProtectedMAE = mean(AvgProtectedMAE),
            AvgProtectedMASE = mean(AvgProtectedMASE),
            AvgSE = mean(SpecEntropy),
            AvgND = mean(AvgNeighborDistance),
            .groups='drop') %>%
  mutate(IdentProp = c(0.090625000, 0.104687500, 0.150000000, 0.075609756, 0.087931034,
                       0.050000000, 0.044054054, 0.086842105, 0.037500000, 0.101851852,
                       0.136111111, 0.023166023, 0.042131980, 0.018965517, 0.061904762, 
                       0.157500000, 0.125000000, 0.123076923, 0.066458333, 0.131250000, 
                       0.092500000, 0.041558442, 0.014864865, 0.036363636, 0.009126984, 
                       0.013157895, 0.117500000, 0.089024390, 0.062337662, 0.009931507))
```


```{r}
tsne_acc_priv %>%
  ggplot(aes(x=AvgND, y=AvgProtectedMASE-AvgMASE)) +
  geom_point()
```

```{r}
summary(lm(AvgProtectedMASE ~ AvgSE, data=tsne_acc_priv))
```
...
```{r}
mm_data <- read.csv("../../Data/Cleaned/M3/monthly-MICRO_h1_train.csv")
qf_data <- read.csv("../../Data/Cleaned/M3/quarterly-FINANCE_h1_train.csv")

mm_protected <- read.csv("../../Data/Cleaned/M3/k-nts-plus_3_monthly-MICRO_h1_train.csv")
qf_protected <- read.csv("../../Data/Cleaned/M3/k-nts-plus_3_quarterly-FINANCE_h1_train.csv")
```
...
mm_protected <- read.csv("../../Data/Cleaned/M3/k-nts-plus_3_monthly-MICRO_h1_train.csv")
qf_protected <- read.csv("../../Data/Cleaned/M3/k-nts-plus-full-normalized_3_quarterly-FINANCE_h1_train.csv")
```

```{r}
apply(qf_data, 1, mean)
```

```{r}
apply(qf_protected, 1, mean)
```
...
```{r fig.width=10, fig.height=10}
mm_data <- as_tibble(t(mm_data)) %>%
  mutate(time=1:n()) %>%
  gather(key='Series', value='value', -time) %>%
  mutate(type='original')

mm_protected <- as_tibble(t(mm_protected)) %>%
  mutate(time=1:n()) %>%
  gather(key='Series', value='value', -time) %>%
  mutate(type='protected')

mm_full <- bind_rows(mm_data, mm_protected)

mm_full %>%
  group_by(Series, type) %>%
  mutate(Series_length = n()) %>%
  ungroup() %>%
  mutate(Series_length = as.character(Series_length)) %>%
  filter(type=='original') %>%
  ggplot(aes(x=time, y=value, color=Series)) +
  geom_line() +
  facet_wrap(~Series_length, scales='free') +
  theme(legend.position='none')
```
