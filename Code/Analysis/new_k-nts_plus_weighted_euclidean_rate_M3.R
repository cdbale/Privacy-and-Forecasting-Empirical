...
perform_knts <- function(ts_file, ts_file_path, seasonal_period, window_length, k, features_to_calculate, selected_features, importance_weights, corr_based=FALSE){
  ...
  X_k <- lapply(Xs, function(x) knts_alg(x, sp=seasonal_period, window_length=window_length, k=k, features_to_calculate=features_to_calculate, selected_features=selected_features, importance_weights=importance_weights, corr_based=corr_based))
...
feature_file_names <- grep("normalized", feature_file_names, value=TRUE, invert=TRUE)
...
for (f in file_names){
  ...
  fsr <- feature_selection(full_data_scaled, num_iter, models=models)
  ...
  for (j in c(3)){
    ...
    X_knts <- perform_knts(ts_file=f,
                           ...
                           importance_weights=fsr$importance_weights)
    
    swap_stop <- Sys.time()
    
    swap_times <- c(swap_times, difftime(swap_stop, swap_start, units="mins"))
    
    write.csv(X_knts, file=paste0(fp, "k-nts-plus-weighted-euclidean_", j, "_", f), row.names=FALSE)
