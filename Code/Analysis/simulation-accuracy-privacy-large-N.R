...
# perform_knts <- function(X, seasonal_period, window_length, k, features_to_calculate, selected_features, corr_based=FALSE){
#   
#   X_k <- knts_alg(time_series=X, sp=seasonal_period, window_length=window_length, k=k, features_to_calculate=features_to_calculate, selected_features=selected_features)
#   
...
# also extract the features from each set of differential privacy baseline protected series
...
# what is the accuracy for each method at the minimum level of privacy

sort(c(20.5, epsvals)[r_dp_pbar_b <= 0.1], decreasing=TRUE)[1]

r_min_privacy_an_param_g <- rowMeans(cbind(r_an_mae_ses_g, r_an_mae_des_g))
r_min_privacy_an_param_g <- (r_min_privacy_an_param_g - r_min_privacy_an_param_g[1])/r_min_privacy_an_param_g[1] * 100
