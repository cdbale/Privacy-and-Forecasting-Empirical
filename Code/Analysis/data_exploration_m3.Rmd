...
***
Look at correlations in subset of wikipedia data.
```{r}
wiki_data <- read.csv("../../Data/Train/Clean/full_wikipedia_clean.csv")

wiki_data <- as.data.frame(t(wiki_data))

cmat_wiki <- cor(wiki_data)

corrs_wiki <- cmat_wiki[upper.tri(cmat_wiki)]

mean(abs(corrs_wiki))

median(abs(corrs_wiki))
...
# original_data <- read.csv("../../Data/Train/Clean/full_wikipedia_clean.csv")
...
ts_features <- tsfeatures(ts_data, features=c("entropy", "stl_features", "acf_features"))
  
f_orig <- ts_features %>% select(entropy, trend, seasonal_strength, x_acf1)
...
```{r}
protected_series_plot("DP_1", 1, c(0,1), ylim(-60000,50000), main_title = "Original Series vs. Differentially Private Series (\u03f5 = 1)")
```

```{r}
protected_series_plot("AN_1", 1, main_title = "Original vs. AN 1")
```
...
```{r}
protected_series_plot("Top_0.4", 1, c(0,1), ylim(0, 10000), main_title = "Original Series vs. Top Coded Series (40%)")
```

```{r}
protected_series_plot("Bottom_0.2", 1, main_title = "Original vs. Bottom 0.2")
```
...
perform_pca <- function(pca_object, protection_method){
  ...
  features <- tsfeatures(td, features=c("entropy", "stl_features", "acf_features"))
...
Get PCA results for strong protection.
```{r}
dp_strong <- perform_pca(f_pca, "DP_1")
an_strong <- perform_pca(f_pca, "AN_2")
top_strong <- perform_pca(f_pca, "Top_0.4")
bottom_strong <- perform_pca(f_pca, "Bottom_0.4")
```
...
adj_results %>%
  ...
  select(Model, Direction, Prop, Magnitude, AvgRelMAE) %>%
  arrange(Model, desc(Direction)) %>%
  filter(Direction %in% c("None"),
         Model %in% c("SES", "DES", "TES"))
```
...
adj_results %>%
  ...
  arrange(Model, desc(Direction)) %>%
  filter(Direction %in% c("Up", "Down")) %>%
  filter(Model %in% c("SES", "DES", "TES"))
```

Make tables with average proportion of adjusted direction, average magnitude of adjustment, and mean AvgRelMAE.
```{r}
adj_results %>%
  filter(Model %in% c("SES", "DES", "TES"), Horizon == "h1", Protection == "Top", Parameter==0.1) %>%
  select(Model, Direction, Prop, Magnitude, AvgRelMAE) %>%
  arrange(Model, desc(Direction)) %>%
  filter(Direction %in% c("Up", "Down"))
```
...
```{r}
# f <- log(rnn_forecasts$`2`)
# 
# rnn <- c(rnn_fitted$`2`, f)
# 
# length(rnn)
# 
# length(dp_1[[snum]])
...
tibble(y = ts_data[[snum]], 
  ...
  labs(y = "log(Y)",
       x = "Time",
       color="",
       title=" RNN and DES Fitted Values from DP (\u03f5 = 1) vs. Original Data") +
  ylim(4, 10)
```
...
extract_es_features <- function(protection_method, original=FALSE){
  ...
  features <- tsfeatures(td, features=c("ses_alpha", "des_alpha", "des_beta", "hw_parameters"), parallel=TRUE)
