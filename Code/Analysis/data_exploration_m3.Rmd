...
```{r}
knts_example1 <- protected_series_plot("k-nts_5", 1, c(0,1), ylim(0, 15000), main_title = "")

knts_example2 <- protected_series_plot("k-nts_10", 1, c(0,1), ylim(0, 15000), main_title = "")

knts_example3 <- protected_series_plot("k-nts_15", 1, c(0,1), ylim(0, 15000), main_title = "")
```

```{r fig.height=6}
library(gridExtra)
plt <- ggarrange(knts_example1, knts_example2, knts_example3,
...
```{r}
alphas <- read_csv("../../Outputs/Results/Model Parameters/ses_alphas.csv")
```
...
alphas <- alphas %>%
  ...
  separate(name, c("Protection", "Parameter"), sep="_") %>%
...
dp_alpha <- alphas %>%
  ...
  mutate(Parameter = factor(Parameter, levels=c("Original", "20", "10", "4.6", "1", "0.1"), labels=c("Original", "\u03f5 = 20", "\u03f5 = 10", "\u03f5 = 4.6", "\u03f5 = 1", "\u03f5 = 0.1"))) %>%
...
top_alpha <- alphas %>%
  ...
  mutate(Parameter = factor(Parameter, levels=c("Original", "0.1", "0.2", "0.4"), labels = c("Original", "Top 0.1", "Top 0.2", "Top 0.4")))  %>%
  ggplot(aes(x=Parameter, y=values)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

knts_alpha <- alphas %>%
  filter(Protection %in% c("knts", "original")) %>%
  mutate(Parameter = factor(Parameter, levels=c("Original", "5", "10", "15"), labels = c("Original", "k = 5", "k = 10", "k = 15")))  %>%
  ggplot(aes(x=Parameter, y=values)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
