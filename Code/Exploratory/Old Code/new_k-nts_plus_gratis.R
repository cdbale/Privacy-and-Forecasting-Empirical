...
generate_target <- function (length = 100, nseries = 10, seasonal_periods = 1, feature_function, 
          target, k = ifelse(length(seasonal_periods) == 1, 3, length(seasonal_periods)), 
          tolerance = 0.05, trace = FALSE, parallel = FALSE) 
{
  tsmatrix <- matrix(NA_real_, nrow = length, ncol = nseries)
  for (i in seq(nseries)) {
    temp <- simulate_target(length = length, seasonal_periods = seasonal_periods, 
                            feature_function = feature_function, target = target, 
                            k = k, tolerance = tolerance, trace = trace, parallel = parallel)
    print(temp)
    print(length(temp))
    tsmatrix[, i] <- temp
  }
...
import_data <- function(file_name, file_path, sp, take_log=TRUE){
...
# 
# temp_data <- import_data("yearly-FINANCE_h1_train.csv", "../../Data/Cleaned/", 1)
# 
...
knts_alg <- function(time_series, augmented_series, sp, window_length, k, features_to_calculate, selected_features, corr_based=FALSE){
  ...
  X_window <- lapply(augmented_series, function(x) ts(x[1:window_length], frequency=sp))
  ...
  ones_column <- as.matrix(rep(1, length(augmented_series)), nrow=length(augmented_series))
  ...
  for (j in 1:num_series){
    ...
    for (t in 1:window_length){
      ...
      # replace the value
      X_new[t,j] <- augmented_series[[i]][t]
  ...
  for (t in (window_length+1):num_periods){
    ...
    X_window <- lapply(augmented_series, function(x) ts(x[(t-window_length+1):t], frequency=sp))
...
for (f in file_names[3:15]){
  ...
  # read in time series
  X <- import_data(file_name=f, file_path=fp, sp=sp)
  ...
  if (file.exists(paste0("../../Data/Cleaned/Augmented/augmented_", f))){
    
    X_augmented <- import_data(file_name=paste0("augmented_", f), file_path="../../Data/Cleaned/Augmented/", sp=sp)
    
    # split X into separate data sets, one for each series length
    Xs_augmented <- list()
    lengths <- sapply(X_augmented, length)
    unique_lengths <- unique(lengths)
    for (l in seq_along(unique_lengths)){
      ids <- lengths==unique_lengths[l]
      Xs_augmented[[l]] <- X_augmented[ids]
    }
    
  } else {
    
    Xs_augmented <- lapply(Xs, function(x) augment_data(x,
                                                        features_to_calculate=fv,
                                                        selected_features=sft,
                                                        num_to_generate=15,
                                                        feature_tol=0.3))
    
    for (x in 1:length(Xs_augmented)){
      names(Xs_augmented[[x]]) <- as.character(c(1:length(Xs_augmented[[x]])))
    }
    
    Xs_augmented1 <- lapply(Xs_augmented, function(x) as.data.frame(t(do.call(cbind, x))))
    
    Xs_augmented1 <- lapply(Xs_augmented1, exp)
    
    Xs_augmented1 <- do.call(rbind.fill, Xs_augmented1)
    
    # create a new sub directory
    if (!file.exists("../../Data/Cleaned/Augmented")){
      dir.create(file.path("../../Data/Cleaned/Augmented"))
    }
    
    # specifying the working directory
    write.csv(Xs_augmented1, file=paste0("../../Data/Cleaned/Augmented/", "augmented_", f), row.names=FALSE)
