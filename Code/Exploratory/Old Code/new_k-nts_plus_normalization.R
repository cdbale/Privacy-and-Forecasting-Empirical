...
# design custom series scaling function
series_scaler <- function(time_series){
  if (var(time_series) > 0){
    ts <- (time_series - mean(time_series))/sd(time_series)
  } else {
    ts <- (time_series - mean(time_series))/.Machine$double.eps
  }
  return(ts)
}
...
knts_alg <- function(time_series, sp, window_length, k, features_to_calculate, selected_features, corr_based=FALSE){
  ...
  X_means <- sapply(time_series, mean)
  X_sds <- sapply(time_series, sd)
  
  X_sds <- ifelse(X_sds == 0, .Machine$double.eps, X_sds)
  
  # normalize time series
  time_series_scaled <- lapply(time_series, series_scaler)
  
  # matrix to hold new series
  X_new <- matrix(0.0, nrow=num_periods, ncol=num_series)
  
  # restrict the data to the beginning window
  X_window <- lapply(time_series_scaled, function(x) ts(x[1:window_length], frequency=sp))
  ...
  for (j in 1:num_series){
    ...
    for (t in 1:window_length){
      ...
      # replace the value
      X_new[t,j] <- time_series_scaled[[i]][t]
  ...
  for (t in (window_length+1):num_periods){
    ...
    X_window <- lapply(time_series_scaled, function(x) ts(x[(t-window_length+1):t], frequency=sp))
...
ed_file_path <- paste0("../../Outputs/Results/Error_Distributions/", data_folder)
...
fv <- c("entropy_c", "lumpiness", "stability",
        ...
        "nonlinearity", "skewness", "kurtosis")
...
feature_file_names <- grep("normalized", feature_file_names, value=TRUE)
...
for (f in file_names){
  ...
  for (ff in current_feature_file_names){
    ...
    if (!params[3] %in% c("AN", "DP")){
      features["Protection"] <- "Original"
      features["Parameter"] <- "Original"
    } else {
      features["Protection"] <- params[3]
      features["Parameter"] <- params[4]
    }
  ...
  if (file.exists(paste0("../../Outputs/RReliefF Rankings/", data_folder))){
    ...
    write.csv(fsr[["evals_combined"]], file=paste0("../../Outputs/RReliefF Rankings/", data_folder, "normalized_RReliefF_", prefix, "_h1_train.csv"), row.names=FALSE)
  ...
  if (file.exists(paste0("../../Outputs/RFE Rankings/", data_folder))){
    ...
    write.csv(fsr[["rank_df"]], file=paste0("../../Outputs/RFE Rankings/", data_folder, "normalized_RFE_", prefix, "_h1_train.csv"), row.names=FALSE)
  ...
  if (file.exists(paste0("../../Outputs/RFE OOB/", data_folder))){
    ...
    write.csv(fsr[["combined_oob"]], file=paste0("../../Outputs/RFE OOB/", data_folder, "normalized_RFE_", prefix, "_h1_train.csv"), row.names=FALSE)
  ...
  for (j in c(3)){
    ...
    write.csv(X_knts, file=paste0(fp, "k-nts-plus-full-normalized_", j, "_", f), row.names=FALSE)
